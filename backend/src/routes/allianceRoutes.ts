...
// @access  Private
router.post('/create', protect, async (req: ExpressRequest, res: ExpressResponse) => {
...
// @access  Private
router.post('/leave', protect, async (req: ExpressRequest, res: ExpressResponse) => {
    const user = req.user!;
    if (!user.allianceId) {
...
// @access  Private
router.get('/chat', protect, async (req: ExpressRequest, res: ExpressResponse) => {
    const user = req.user!;
    if (!user.allianceId) {
        return res.status(400).json({ message: "Nie należysz do żadnego sojuszu." });
    }
...
// @access  Private
router.post('/chat', protect, async (req: ExpressRequest, res: ExpressResponse) => {
    const user = req.user!;
